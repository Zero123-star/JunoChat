...
import { createCharacter, createCharacterAPI } from '@/api';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Creating character with:', {
        name: formData.name,
        persona: formData.description,
        image_url: formData.avatar,
        // ...other fields
      });

      // 1. Create character in Django backend
      await createCharacter({
        formData,
        creator_id: localStorage.getItem('user'),
      });

      // 2. Create character in FastAPI backend
